...
import { spreadTimes, toUtcDate, todayInZone } from './time';
...
export const generateDoseRecordsForToday = async (userId: string): Promise<void> => {
  const schedules = await prisma.medicationSchedule.findMany({ where: { userId, medication: { isActive: true } } });
...
export const refreshMissed = async (userId: string): Promise<void> => {
  ...
  await prisma.doseRecord.updateMany({
    where: { userId, status: DoseStatus.pending, scheduledAt: { lt: threshold } },
